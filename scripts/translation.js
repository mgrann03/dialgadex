 ...
 */
function TranslatedSpeciesName(id, fallback, shadow = false) {
    ...
    if (fallback) {
        ...
        if (shadow) {
            const shadow_html = `<span class=shadow-text>${GetTranslation("terms.shadow", "Shadow")}</span>`;
            trans = FormatTranslation("terms.shadow-order", {
                shadow: shadow_html,
                species: trans
            }, "{shadow} {species}");
        }
